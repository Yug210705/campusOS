...
import { Search, Plus, MapPin, Clock, AlertCircle, Phone, X, Camera, UploadCloud } from "lucide-react";
...
export default function LostAndFoundPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newItem = {
      ...
      location,
      time: "Just now",
      image: itemImage || "https://images.unsplash.com/photo-1584438784894-089d6a62b8fa?q=80&w=600&auto=format&fit=crop", // Fallback placeholder
      status: reportType,
      reporter: "Yug Pathak", // Using local user name
    };
    ...
    // Switch to the tab where the item was reported
      ...
      <div className="p-3 flex flex-col gap-3">
        <AnimatePresence>
          {displayedItems.map((item, idx) => (
            ...
            >
              {/* Image Container */}
              <div className="w-24 h-24 rounded-xl bg-slate-100 overflow-hidden relative shrink-0 border border-slate-200/50">
            ...
            >
              ...
              <div className="p-5 border-t border-slate-100 bg-white">
                <button 
                  type="submit"
                  form="report-form"
                  className="w-full py-4 rounded-xl bg-indigo-600 text-white text-sm font-bold shadow-lg shadow-indigo-600/20 active:scale-[0.98] transition-transform flex items-center justify-center gap-2"
                >
                  Submit Report
                </button>
              </div>
