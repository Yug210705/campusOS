...
import { ChevronLeft, Share, Bookmark, Phone, MessageSquare, MapPin, Clock, User, Check, Loader2, AlertCircle } from "lucide-react";
import Link from "next/link";
import { getLostFoundItemById, resolveLostFoundItem } from "@/actions/dbActions";
import { useAuth } from "@/context/AuthContext";

export default function LostFoundItemDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function loadItem() {
      try {
        ...
        const fetchedItem = await getLostFoundItemById(unwrappedParams.id, isGuest);
        setItem(fetchedItem);
      } catch (err) {
        console.error("Failed to load lost/found item:", err);
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleResolve = async () => {
    ...
    try {
      const isGuest = authUser?.isAnonymous ?? true;
      if (isGuest || !item._id.match(/^[0-9a-fA-F]{24}$/)) {
        // Mock success for guest or mock item ids
        alert("Item marked as resolved successfully!");
      } else {
        await resolveLostFoundItem(item._id);
      }
      router.push("/connect/lost-found");
    } catch (err) {
  ...
  if (!item) {
    return (
      <div className="min-h-screen bg-slate-50 max-w-md mx-auto flex flex-col items-center justify-center p-6 text-center">
        ...
        <h3 className="font-bold text-slate-800 text-sm">Report Not Found</h3>
        <p className="text-xs text-slate-500 mt-1 max-w-[200px] mb-6">
          This item may have been returned or resolved.
        </p>
  ...
  return (
    <div className="min-h-screen bg-slate-50 max-w-md mx-auto relative pb-28">
      ...
      <div className="w-full h-80 bg-slate-100 border-b border-slate-200 overflow-hidden relative">
        ...
        {/* Status Badge */}
        <div className="absolute bottom-4 left-4 px-3 py-1.5 rounded-xl bg-white shadow-sm flex items-center gap-1.5 border border-slate-100">
          {item.status === "lost" ? (
            <AlertCircle className="w-4 h-4 text-rose-500" />
          ) : (
            <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
          )}
          <span className={`text-[10px] font-black uppercase tracking-widest leading-none ${item.status === "lost" ? "text-rose-600" : "text-emerald-600"}`}>
      ...
      <main className="p-5 flex flex-col gap-5">
        ...
        <div className="bg-white rounded-3xl p-4 border border-slate-200 shadow-sm flex flex-col gap-3">
          <div className="flex items-center gap-3 text-xs font-semibold text-slate-700">
            ...
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider leading-none mb-1">Reported By</p>
              <p className="font-bold text-slate-800 leading-tight">
                {item.reporter} <span className="text-[9px] font-semibold text-slate-400">({item.status === "lost" ? "Student" : "Staff/Finder"})</span>
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 text-xs font-semibold text-slate-700">
            ...
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider leading-none mb-1">
                {item.status === "lost" ? "Last Seen Location" : "Found Location"}
              </p>
          ...
          <div className="flex items-center gap-3 text-xs font-semibold text-slate-700">
            ...
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider leading-none mb-1">Time Reported</p>
              <p className="font-bold text-slate-800 leading-tight">{item.time || "Just now"}</p>
            </div>
          </div>
        </div>
      ...
      <footer className="p-4 bg-white border-t border-slate-100 sticky bottom-0 z-30 flex flex-col gap-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <a href={`tel:${item.contactNumber || "9876543210"}`} className="col-span-1">
          ...
          <a href={`sms:${item.contactNumber || "9876543210"}`} className="col-span-1">
            <button className="w-full py-3.5 bg-indigo-50 hover:bg-indigo-100 border border-indigo-100 text-indigo-700 rounded-xl text-xs font-bold active:scale-[0.98] transition-transform flex items-center justify-center gap-2">
              <MessageSquare className="w-4 h-4" /> Message Reporter
            </button>
          </a>
        </div>
